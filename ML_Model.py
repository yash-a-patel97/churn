...
# Define a function to load the model and scaler
...
if dataset_name == 'Iris':
    ...
    if st.button('Predict Iris Species', key='predict_iris'):
        iris_features = np.array([sepal_length, sepal_width, petal_length, petal_width]).reshape(1, -1)
        iris_features_scaled = iris_scaler.transform(iris_features)
    ...
    if st.button('Predict Wine Authenticity', key='predict_wine'):
        ...
        wine_features_scaled = wine_scaler.transform(wine_features)
